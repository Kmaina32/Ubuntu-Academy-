...
export const runContentStrategy = ai.defineFlow(
  ...
  async () => {
    ...
    const ideasResponse = await ai.generate({
      prompt: `You are a Content Strategist for SkillSet Academy, an online learning platform for a Kenyan audience. Your task is to brainstorm 10 highly relevant and marketable course ideas that would appeal to students looking to upskill. Provide a title, a category, and a plausible instructor name for each.`,
