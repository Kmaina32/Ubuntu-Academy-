...
import { getAllCourses, getAllCalendarEvents, getAllNotifications, getUserById, getLiveSession, saveUser, updateInvitationStatus, logActivity, getHeroData } from '@/lib/firebase-service';
  ...
  useEffect(() => {
      if (!loading && user) {
          ...
          if (activityTrackingEnabled) {
              logActivity(user.uid, { type: 'page_visit', details: { path: pathname } });
          }
      }
  ...
  return (
    <header className="flex h-16 items-center border-b bg-background px-4 md:px-6 sticky top-0 z-30">
        ...
        <div className="flex w-full items-center justify-end gap-2">
            <ThemeToggle />
            {loading ? (
            <Skeleton data-testid="auth-skeleton" className="h-8 w-8 rounded-full" />
            ) : user ? (
            <>
